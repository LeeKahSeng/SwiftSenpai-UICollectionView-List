//
//  NormalHeaderReloadValViewController.swift
...
class NormalHeaderReloadValViewController: UIViewController {
...
extension NormalHeaderReloadValViewController: UICollectionViewDelegate {
    
    func collectionView(_ collectionView: UICollectionView,
                        didSelectItemAt indexPath: IndexPath) {
        
        var newSnapshot = dataSource.snapshot()
        let oldParent = newSnapshot.sectionIdentifiers[indexPath.section]
        
        // Change parent's title to selected cell's title
        var newParent = oldParent
        newParent.title = newSnapshot.itemIdentifiers(inSection: oldParent)[indexPath.item].title
        ...
        // Replace `oldParent` with `newParent` (using insert and delete)
        newSnapshot.insertSections([newParent], afterSection: oldParent)
        newSnapshot.deleteSections([oldParent])

        // Reconstruct the new section
        newSnapshot.appendItems(newParent.children, toSection: newParent)
